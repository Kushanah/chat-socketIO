...
const moduleDB = require("./module/user");
...
app.get("/logout", function(req, res) {
    req.session.destroy((err) => {
        if(!err) {
            res.render("login.ejs", {errors : "logout"});
        } else {
            res.send(403);
        }
    });
...
app.post("/login", function(req, res) {
    moduleDB.getCredential(req.body.pseudo, db, (r) => {
        if(bcrypt.compareSync(req.body.password, r.password)) {
            req.session.accountid = r.id;
            res.redirect("/");
        } else {
            res.render("login.ejs", {errors : "incorrect"});
        }
    });
});
    ...
    if(req.body.pseudo && req.body.password) {
        moduleDB.checkPseudo(req.body.pseudo, db, (r) => {
            if(r) {
                ...
                res.render("register.ejs", {errors : "name"});
            }
        });
